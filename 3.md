...
flowchart TD
    ...
    KAFKA --> CACHE_WORKER[Cache Builder]
    CACHE_WORKER --> REDIS
    ...
    ADMIN[Admin Portal] -->|Upload| UPLOAD[Upload Service]
    UPLOAD --> S3_RAW[(S3 Ingest)]
    UPLOAD --> KAFKA
...
*Flow shown in main HLD: Admin Portal → Upload Service → S3 → Kafka → Transcoding → S3 → Kafka → Workers*

#### 7.1 Step-by-Step Explanation

| Step | Component | What Happens |
|------|-----------|--------------|
| 1 | Admin Portal | Content team uploads 4K master video + metadata (title, cast, genres) |
| 2 | Upload Service | Validates file, generates titleId, stores raw video in S3 Ingest |
| 3 | Kafka | Publishes `VIDEO_UPLOADED` event |
| 4 | Transcoding Workers | Encodes into 24 variants (4 resolutions × 2 codecs × 3 audio) |
| 5 | Transcoding Workers | Splits into 4-sec chunks, encrypts with DRM, generates HLS manifests |
| 6 | S3 Videos | Stores encoded chunks and manifests |
| 7 | Kafka | Publishes `CONTENT_READY` event |
| 8 | Cache Builder | Updates PostgreSQL → Redis → Elasticsearch |
| 9 | CDN Worker | Pushes to Open Connect appliances (proactive fill) |
| 10 | ML Worker | Generates content embedding for recommendations |

#### 7.2 Transcoding Output

```
Input:  source.mp4 (4K master, ~50GB)

Output: 24 variants = 4 Resolutions × 2 Codecs × 3 Audio
        
        s3://netflix-videos/{titleId}/
        ├── master.m3u8
        ├── 480p_h264/
        │   ├── playlist.m3u8
        │   └── chunk_0001.ts, chunk_0002.ts, ...
        ├── 1080p_h265/
        └── 4k_h265/
```
...
| Question | Answer |
|----------|--------|
| Why Kafka after Upload? | Decoupling — API returns immediately, transcoding runs async |
| Why 24 variants? | Different devices need different codecs/resolutions |
| Why 4-second chunks? | Enables adaptive bitrate switching |
| Why proactive CDN fill? | Popular content at edge before users request |
...
*Flow shown in main HLD: Kafka → Spark → Data Lake → ML Training → Feature Store/Model Registry → Recommendation Service*

#### 9.1 Two Parts: Offline + Online

| Part | When | What |
|------|------|------|
| **Offline** | Daily 2 AM | Spark processes watch events → computes features → trains models |
| **Online** | Per request | Fetch features → generate candidates → rank → return |

---

#### 9.2 Offline Pipeline (Daily Batch)

| Step | Component | Output |
|------|-----------|--------|
| 1 | Kafka → Data Lake | Watch events stored in S3 |
| 2 | Spark | Aggregates user behavior |
| 3 | Feature Engineering | User features + Title features |
| 4 | Model Training | ALS, Content-Based NN, Sequence Model |
| 5 | Feature Store | Stores features for online lookup |
| 6 | Model Registry | Stores trained models |

**Features Computed:**

| User Features | Title Features |
|---------------|----------------|
| watch_history_embedding (128 dims) | content_embedding (256 dims) |
| genre_affinity {action: 0.35, ...} | popularity_score |
| avg_session_length | avg_completion_rate |
| time_pattern {evening: 0.6, ...} | similar_titles |

---

#### 9.3 Online Serving (Per Request)

**Request:** `GET /recommendations/{profileId}`

| Step | Action | Result |
|------|--------|--------|
| 1 | Check Redis cache | HIT (90%) → return in 2ms |
| 2 | Get user features | Feature Store → embedding, history, preferences |
| 3 | Generate candidates | 500 titles from: ALS + Content + Trending + Genre |
| 4 | Rank candidates | Neural model scores each candidate |
| 5 | Post-process | Remove watched, apply diversity, filter region |
| 6 | Cache & return | Redis SET (TTL 1hr), return top 50 |

**Candidate Sources:**

| Source | How | Titles |
|--------|-----|--------|
| ALS (Collaborative) | Similar users' titles | ~200 |
| Content-Based | Similar to watched | ~150 |
| Trending | Popular now | ~100 |
| Genre | Top in preferred genres | ~100 |
| **Total (deduplicated)** | Remove duplicates + watched | ~500 |

**Post-Processing Rules:**

| Rule | Purpose |
|------|---------|
| Remove watched | Don't recommend what user saw |
| Max 5 per genre | Ensure diversity |
| Region filter | Only available titles |
| Maturity filter | Respect profile settings |
...
| Question | Answer |
|----------|--------|
| Why cache? | Computing is expensive (~100ms), cache serves in 2ms |
| Why Feature Store? | Pre-computed features enable fast inference |
| Why multiple candidate sources? | Single source lacks diversity |
| Why post-process? | ML optimizes relevance, rules ensure diversity |
...
|--|-----------------|----------------------|
| **Data** | Kafka → S3 Data Lake | Feature Store |
| **Processing** | Spark batch | Real-time inference |
| **Output** | Features + Models | Ranked recommendations |
| **Latency** | Hours | 2ms (hit) / 100ms (miss) |
